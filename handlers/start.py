...
def register(app: Client):
    ...
    async def quota(client: Client, message: Message):
        ...
        limit = Config.FREE_UPLOADS_PER_DAY if plan == "free" else -1
        await message.reply(Messages.quota_text(used, limit if limit != -1 else "∞", plan), reply_markup=Keyboards.back_to_start(), parse_mode="html")
    ...
    async def cb_quota(client, cq: CallbackQuery):
        ...
        plan = user.plan.value if user else "free"
        limit = Config.FREE_UPLOADS_PER_DAY if plan == "free" else -1
        await cq.message.edit_text(Messages.quota_text(used, limit if limit != -1 else "∞", plan), reply_markup=Keyboards.back_to_start(), parse_mode="html")
